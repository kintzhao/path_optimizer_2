...
bool ReferencePathSmoother::graphSearchDp(std::shared_ptr<PathOptimizationNS::ReferencePath> reference) {
    auto t1 = std::clock();
    ...
    for (int i = 0; i < layers_s_list_.size(); ++i) {
        ...
        while (cur_l <= FLAGS_search_lateral_range) {
            ...
            if (i == 0 && dp_point.lateral_index == start_lateral_index) {
                ...
                dp_point.dir = start_state_.heading;
                dp_point.cost = 0.0;
            }
    ...
    for (const auto &layer : samples) {
        ...
        for (const auto &point : layer) {
            calculateCostAt(samples, point.layer_index, point.lateral_index);
...
bool ReferencePathSmoother::graphSearch(std::shared_ptr<PathOptimizationNS::ReferencePath> reference) {
    ...
    start_point.dir = proj_point.heading;
